...
import { customer, orderCaptured, orderDispatched, vender } from '../constants/userType';
    ...
    if (user) {
      if (user.userType == customer) {
        if (Array.isArray(customerOrder.orders) && customerOrder.orders.length) {
          return customerOrder.orders.filter((order) => order.status === orderCaptured)
        }
      } else {
        if (Array.isArray(vendorOrder.orders) && vendorOrder.orders.length) {
          return vendorOrder.orders.filter((order) => order.status === orderCaptured)
        }
      } 
    }
